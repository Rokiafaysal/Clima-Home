...
class AsyncFunction {
  ...
  static void startAutoMode({required String device,required bool isAutoMode,required int time,required Future<void> Function() task,}) {
    if (isAutoMode) {
      if (_runningStates[device] == true) return;
      _runningStates[device] = true;
      print("Auto mode started for $device");

      _timers[device] = Timer.periodic(Duration(seconds: time), (timer) async {
